    ...
    const fetchData = async (p: number) => {
      ...
      try {
        ...
        if (res.code === 200) {
          ...
          addToast(res.message || "Failed to load history.", { type: "error", title: "Error" });
        }
      } catch {
  ...
  return (
    <main className="mx-auto max-w-4xl p-6">
      <h1 className="text-2xl font-bold mb-4 inline-flex items-center gap-2">
        <img src="/icon/history.svg" alt="history" style={{ width: 24, height: 24 }} />
        My History
      </h1>
      ...
      {loading ? (
        ...
        <div className="rounded border bg-gray-50 p-6 text-gray-500">No history yet.</div>
      ) : (
